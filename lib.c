...
int criaTarefa(ListaDeTarefas *lt) {
  ...
  do {
    ...
    printf("1. Trabalho\n2. Estudos\n3. Familia/Amigos\n4. Pessoal\n5. Outros\n");
    scanf("%d", &t->categoria);
    if (t->categoria < 1 || t->categoria > 5){printf("Erro: valor inválido, digite um numero entre 1 a 5\n");}
  ...
  t->estado = 0; // DEFINE COMO PADRÃO PARA AS TAREFAS CRIADAS O ESTADO '0'(Não iniciada)
...
int listaTarefas(ListaDeTarefas *lt) {
  ...
  for (int i = 0; i < lt->qtd; i++) {
    ...
    printf("\n--- TAREFA %d ---\n", i);
    if (t->categoria == 1) {printf("CATEGORIA: Trabalho\n");} 
    else if (t->categoria == 2) {printf("CATEGORIA: Estudos\n");} 
    else if (t->categoria == 3) {printf("CATEGORIA: Familia/Amigos\n");}
    else if (t->categoria == 4) {printf("CATEGORIA: Pessoal\n");}
    else if (t->categoria == 5) {printf("CATEGORIA: Outros\n");}
    else {printf("!!!CATEGORIA INVALIDA.\n");}
    printf("PRIORIDADE: %d\n", t->prioridade);
    printf("DESCRICAO: %s\n", t->descricao);
    if (t->estado == 0) {printf("ESTADO: Nao iniciado\n");}
    else if (t->estado == 1) {printf("ESTADO: Em andamento\n");}
    else if (t->estado == 2) {printf("ESTADO: Concluido\n");}
...
int alterarTarefa(ListaDeTarefas *lt) {
  ...
  do {
    printf("1. Categoria\n2. Prioridade\n3. Descricao\n4. Estado\n");
    scanf("%d", &opcao);
    if (opcao < 1 || opcao > 4) {printf("Opcao invalida, digite um numero de 1 a 4.\n");}
  } while (opcao < 1 || opcao > 4);
  if (opcao == 1) {
    printf("\nESCOLHA A NOVA CATEGORIA DA TAREFA:\n1. Trabalho\n2. Estudos\n3. Familia/Amigos\n4. Pessoal\n5. Outros\n");
    scanf("%d", &lt->tarefas[posTarefa].categoria);
  } else if (opcao == 2) {
    printf("\nDIGITE A NOVA PRIORIDADE DA TAREFA (0 - Mais importante // 10 - menos importante):\n");
...
int filtrarTarefas(ListaDeTarefas *lt) {
  ...
  if (opcao == 1) {
    ...
    printf("\nDIGITE A PRIORIDADE QUE DESEJA VER (0 - Mais importante // 10 - menos importante):\n");
    scanf("%d", &auxPri);
    for (int i = 0; i < lt->qtd; i++) {
      Tarefa *t = &lt->tarefas[i];
    ...
    do {
      ...
      printf("0. Nao iniciado\n1. Em andamento\n2. Concluido\n");
      scanf("%d", &auxEst);
      if (auxEst < 0 || auxEst > 2) {printf("Opcao invalida, digite um numero entre 0 e 2\n");}
    ...
    do {
      ...
      printf("1. Trabalho\n2. Estudos\n3. Familia/Amigos\n4. Pessoal\n5. Outros\n");
      scanf("%d", &auxCat);
      if (auxCat < 1 || auxCat > 5) {printf("Opcao invalida, digite um numero entre 1 e 5\n");}
    } while (auxCat < 1 || auxCat > 5);
    for (int i = 0; i <= 10; i++) { // A VARIÁVEL 'i' SERÁ UTILIZADA PARA ITERAR SOBRE AS POSSÍVEIS PRIORIDADES DAS TAREFAs (DE 0 A 10)
...
int exportarTarefas(ListaDeTarefas *lt) {
  ...
  do {
    ...
    scanf("%d", &opcao);
    if (opcao < 1 || opcao > 3) {printf("Opcao invalida, digite um numero entre 1 e 3.\n");}
  } while (opcao < 1 || opcao > 3);
  if (opcao == 1 || opcao == 3) {
    do {
      printf("DIGITE A PRIORIDADE DAS TAREFAS QUE DESEJA EXPORTAR (0 a 10):\n");
      scanf("%d", &auxPrioridade);
      if (auxPrioridade < 0 || auxPrioridade > 10) {printf("Opcao invalida, digite um número entre 0 e 10.\n");}
  ...
  if (opcao == 2 || opcao == 3) {
    do {
      ...
      if (auxCategoria < 1 || auxCategoria > 5) {printf("Opcao invalida, digite um numero entre 1 e 5\n");}
  ...
  for (int i = 0; i < lt->qtd; i++) {
    ...
    if (opcao == 1) {
      auxBoolCategoria = 1;
      auxBoolPrioridade = 0;
      if (t->prioridade == auxPrioridade) {auxBoolPrioridade = 1;}
    } else if (opcao == 2) {
      auxBoolCategoria = 0;
      auxBoolPrioridade = 1;
      if (t->categoria == auxCategoria) {auxBoolCategoria = 1;}
    } else if (opcao == 3) {
      auxBoolCategoria = 0;
      auxBoolPrioridade = 0;
      if (t->prioridade == auxPrioridade) {auxBoolPrioridade = 1;}
